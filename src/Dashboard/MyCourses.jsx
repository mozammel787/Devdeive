...
import PrivetCard from "../Components/PrivetCard";
...
const MyCourses = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/course/${user?.email}`
        );
  ...
  return (
    <div className="container mx-auto my-36  p-5 sm:p-0">
      <h2 className=" font-semibold text-5xl text-center ">My Courses</h2>
      ...
      <div className="container mx-auto flex flex-wrap items-center justify-between gap-5">
        {filteredData.map((item, i) => (
          <PrivetCard key={i} course={item}></PrivetCard>
        ))}
      </div>
    </div>
  );
